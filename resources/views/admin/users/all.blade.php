                            ...
                            @foreach($users as $user)
                                <tr>
                                    ...
                                    <td class="d-flex">
                                        ...
                                        @endcan
                                        @can('edit-user')
                                                <a href="{{ route('admin.users.edit', ['user' => $user->id]) }}" class="btn btn-sm btn-outline-primary mr-1">ویرایش</a>
                                        @endcan
                                        @if($user->isStaffUser())
                                            @can('staff-user-permissions')
                                                    <a href="{{ route('admin.users.permissions', $user->id) }}" class="btn btn-sm btn-warning mr-1">دسترسی ها</a>
                                            @endcan
                                        @endif
                                    </td>
                                </tr>
